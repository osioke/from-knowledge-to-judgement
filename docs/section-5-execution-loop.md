...
![The Execution Loop Flywheel — a circular diagram showing: Deconstruct → Framework → Execute → Spar → Capture Recipe → back to Deconstruct](images/fig5-execution-loop.png)

<p class="figure-caption">Figure 5. The Execution Loop Flywheel. The system is self-improving: execution generates Recipes, Recipes reduce TTLS, reduced TTLS brings more practitioners into the loop, and more practitioners generate more diverse scenarios that sharpen the frameworks for everyone.</p>
